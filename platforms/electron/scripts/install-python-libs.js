...
const BACKEND_DIR = path.join(__dirname, '..', '..', 'web', 'backend');
...
async function checkPython() {
    ...
    if (!fs.existsSync(PYTHON_EXE)) {
        ...
        console.log('\n💡 请先运行:');
        console.log('   npm run install-python\n');
        return false;
    }
...
async function installPackages() {
    ...
    // 嵌入式 Python 的 pip 是 .py 文件，不是 .exe
    const pipPath = process.platform === 'win32'
        ? path.join(SITE_PACKAGES, 'pip', '__main__.py')
