...
function handleWrongbook(url, options) {
    ...
    if (method === 'POST') {
        ...
        if (!exists && body.question) {
            body.question.wrong_count = 1;
            body.question.last_wrong_time = new Date().toLocaleString('zh-CN');
            wrongbook.push(body.question);
    ...
    if (method === 'DELETE') {
        ...
        if (id && id !== 'wrongbook') {
            wrongbook = wrongbook.filter(function(q) { return q.id !== id; });
        } else {
            wrongbook = [];
        }
        localStorage.setItem('quiz_wrongbook', JSON.stringify(wrongbook));
